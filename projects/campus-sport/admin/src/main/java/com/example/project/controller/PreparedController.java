...
import com.example.project.entity.Prepared;
...
public class PreparedController {
    ...
    @Resource
    private IPreparedService preparedService;

    @AutoLog("新增预选场地")
    @PostMapping
    public Result save(@RequestBody Prepared prepared) {
	

        User user = SessionUtils.getUser();
        prepared.setUserId(user.getId());
		
        preparedService.save(prepared);
        return Result.success();
    }

    @AutoLog("编辑预选场地")
    @PutMapping
    public Result update(@RequestBody Prepared prepared) {
        preparedService.updateById(prepared);
    ...
    public Result deleteBatch(@RequestBody List<Integer> ids) {
        preparedService.removeByIds(ids);
        return Result.success();
    }
    ...
    */
    ...
    public void export(HttpServletResponse response) throws Exception {
        // 从数据库查询出所有的数据
        List<Prepared> list = preparedService.list();
        // 在内存操作，写出到浏览器
        ...
        String fileName = URLEncoder.encode("Prepared信息表", "UTF-8");
    ...
    */
    ...
    public Result imp(MultipartFile file) throws Exception {
        ...
        List<Prepared> list = reader.readAll(Prepared.class);

        preparedService.saveBatch(list);
        return Result.success();
    }
