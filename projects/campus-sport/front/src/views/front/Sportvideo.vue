<script setup>
  ...
  const load = (key) => {
    ...
    request.get('/front/sportvideo/page', {
      params: {
        name: name,
  ...
  <div>
    ...
    <div style="width:85%;margin: 0 auto;margin-bottom: 50px;">
        <div style="padding-bottom: 15px ;margin-top: 20px;text-align: left;">
          <span style="font-size: 14px;margin-right: 20px;">当前位置：首页 > 运动教学</span>
        </div>

        <div style="padding-bottom: 15px ;border-bottom: 3px solid #0099CC; text-align: left;display: flex;">
          <span style="font-weight: bold; font-size: 24px;float: left;flex: 3;color: #0099CC;">运动教学</span>

          <div style="margin-top: 10px;float: right;flex: 1;">
            <el-input style="width: 200px" placeholder="查询运动教学" v-model="state.searchKey" clearable></el-input>
            ...
            <div>
              <el-row :gutter="10">
                <el-col :span="6" v-for="item in state.tableData" :key="item.id" style="margin-top: 20px;">
                    <div ><img @click="router.push('/front/sportvideo-detail?id=' + item.id)" :src="item.img" alt="" style="width: 95%; height: 180px;cursor: pointer;"></div>
                    <div><span style="font-weight: bold">{{ item.name}}</span></div>
                    <div style="margin: 5px 0; text-align: right">
                      <el-button type="primary" size="small" @click="router.push('/front/sportvideo-detail?id=' + item.id)" style="float: left;">查看详情</el-button>
