...
const confirmDelBatch = () => {
  ...
  request.post('/books/del/batch', idArr).then(res => {
    if (res.code === '200') {
...
// 导出接口
const exportData = () => {
  window.open(`http://${config.serverUrl}/books/export`)
}
...
<template>
  <div style="background-color: #ffffff;padding: 10px;border-radius: 10px;margin-top: 20px;">
    ...
    <div style="margin: 10px 0">
      <el-button type="success" @click="handleAdd" v-if="auths.includes('books.add')">
        <el-icon style="vertical-align: middle">
          <Plus />
        </el-icon>  <span style="vertical-align: middle"> 添加 </span>
      </el-button>
      <el-upload
          v-if="auths.includes('books.import')"
          class="ml5"
          :show-file-list="false"
          style="display: inline-block; position: relative; top: 3px"
          :action='`http://${config.serverUrl}/books/import`'
      ...
      <el-button type="primary" @click="exportData" class="ml5" v-if="auths.includes('books.export')">
        <el-icon style="vertical-align: middle">
          <Top />
        </el-icon>  <span style="vertical-align: middle"> 导出 </span>
      </el-button>
      <el-popconfirm title="您确定删除吗？" @confirm="confirmDelBatch" v-if="auths.includes('books.deleteBatch')">
        <template #reference>
    ...
    <div style="margin: 10px 0">
      ...
      <el-table-column label="预约明细"><template #default="scope"><el-button @click="view(scope.row.content)">查看</el-button></template></el-table-column>
      <el-table-column prop="stateRadio" label="订单状态"></el-table-column>
      <el-table-column label="预约用户"><template #default="scope"><span v-if="scope.row.userId">{{ state.userOptions.find(v => v.id === scope.row.userId) ? state.userOptions.find(v => v.id === scope.row.userId).name : '' }}</span></template></el-table-column>
      <el-table-column prop="goodids" label="场地编号"></el-table-column>
      <el-table-column prop="intime" label="进场时间"></el-table-column>
      <el-table-column prop="outtime" label="离场时间"></el-table-column>

        <el-table-column label="操作" width="220">
          <template #default="scope">
            <el-button type="primary" @click="handleEdit(scope.row)" v-if="auths.includes('books.edit')">修改<el-icon style="vertical-align: middle"><Edit /></el-icon></el-button>
            <el-popconfirm title="您确定删除吗？" @confirm="del(scope.row.id)" v-if="auths.includes('books.delete')">
              <template #reference>
    ...
    <el-dialog v-model="dialogFormVisible" title="体育场预定信息" width="40%">
      <el-form ref="ruleFormRef" :rules="rules" :model="state.form" label-width="80px" style="padding: 0 20px" status-icon>
        <el-form-item label-width="100" prop="name" label="预约号">
          <el-input v-model="state.form.name" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label-width="100" prop="content" label="预约明细">
    ...
    />
        ...
        <el-form-item label-width="100" prop="goodids" label="场地编号">
          <el-input v-model="state.form.goodids" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label-width="100" prop="intime" label="进场时间">
          <el-date-picker style="width: 100%" v-model="state.form.intime" type="datetime" value-format="YYYY-MM-DD HH:mm:ss" placeholder="选择日期时间"></el-date-picker>
        </el-form-item>
        <el-form-item label-width="100" prop="outtime" label="离场时间">
