...
const confirmDelBatch = () => {
  ...
  request.post('/rotation/del/batch', idArr).then(res => {
    if (res.code === '200') {
...
// 删除
const del = (id) => {
  request.delete('/rotation/' + id).then(res => {
    if (res.code === '200') {
      ElMessage.success('操作成功')
      load()  // 刷新表格数据
    } else {
...
// 导出接口
const exportData = () => {
  window.open(`http://${config.serverUrl}/rotation/export`)
}
...
<template>
  <div style="background-color: #ffffff;padding: 10px;border-radius: 10px;margin-top: 20px;">
    ...
    <div style="margin: 10px 0">
      <el-button type="success" @click="handleAdd" v-if="auths.includes('rotation.add')">
        <el-icon style="vertical-align: middle">
          <Plus />
        </el-icon>  <span style="vertical-align: middle"> 添加 </span>
      </el-button>
      <el-upload
          v-if="auths.includes('rotation.import')"
          class="ml5"
          :show-file-list="false"
          style="display: inline-block; position: relative; top: 3px"
          :action='`http://${config.serverUrl}/rotation/import`'
      ...
      <el-button type="primary" @click="exportData" class="ml5" v-if="auths.includes('rotation.export')">
        <el-icon style="vertical-align: middle">
          <Top />
        </el-icon>  <span style="vertical-align: middle"> 导出 </span>
      </el-button>
      <el-popconfirm title="您确定删除吗？" @confirm="confirmDelBatch" v-if="auths.includes('rotation.deleteBatch')">
        <template #reference>
      ...
      <el-table-column prop="indexRadio" label="是否首页"></el-table-column>
        ...
        <el-table-column label="操作" width="220">
          <template #default="scope">
            <el-button type="primary" @click="handleEdit(scope.row)" v-if="auths.includes('rotation.edit')">修改<el-icon style="vertical-align: middle"><Edit /></el-icon></el-button>
            <el-popconfirm title="您确定删除吗？" @confirm="del(scope.row.id)" v-if="auths.includes('rotation.delete')">
              <template #reference>
