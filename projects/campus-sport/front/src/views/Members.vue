...
const confirmDelBatch = () => {
  ...
  request.post('/members/del/batch', idArr).then(res => {
    if (res.code === '200') {
...
// 删除
const del = (id) => {
  request.delete('/members/' + id).then(res => {
    if (res.code === '200') {
      ElMessage.success('操作成功')
      load()  // 刷新表格数据
    } else {
...
// 导出接口
const exportData = () => {
  window.open(`http://${config.serverUrl}/members/export`)
}
...
<template>
  <div style="background-color: #ffffff;padding: 10px;border-radius: 10px;margin-top: 20px;">
    ...
    <div style="margin: 10px 0">
      <el-button type="success" @click="handleAdd" v-if="auths.includes('members.add')">
        <el-icon style="vertical-align: middle">
          <Plus />
        </el-icon>  <span style="vertical-align: middle"> 添加 </span>
      </el-button>
      <el-upload
          v-if="auths.includes('members.import')"
          class="ml5"
          :show-file-list="false"
          style="display: inline-block; position: relative; top: 3px"
          :action='`http://${config.serverUrl}/members/import`'
      ...
      <el-button type="primary" @click="exportData" class="ml5" v-if="auths.includes('members.export')">
        <el-icon style="vertical-align: middle">
          <Top />
        </el-icon>  <span style="vertical-align: middle"> 导出 </span>
      </el-button>
      <el-popconfirm title="您确定删除吗？" @confirm="confirmDelBatch" v-if="auths.includes('members.deleteBatch')">
        <template #reference>
      ...
      <el-table-column prop="sexRadio" label="性别"></el-table-column>
        ...
        <el-table-column label="操作" width="220">
          <template #default="scope">
            <el-button type="primary" @click="handleEdit(scope.row)" v-if="auths.includes('members.edit')">修改<el-icon style="vertical-align: middle"><Edit /></el-icon></el-button>
            <el-popconfirm title="您确定删除吗？" @confirm="del(scope.row.id)" v-if="auths.includes('members.delete')">
              <template #reference>
    ...
    <el-dialog v-model="dialogFormVisible" title="用户信息" width="40%">
      <el-form ref="ruleFormRef" :rules="rules" :model="state.form" label-width="80px" style="padding: 0 20px" status-icon>
        ...
        <el-form-item label-width="100" prop="age" label="年龄">
          <el-input v-model="state.form.age" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label-width="100" prop="sexRadio" label="性别">
